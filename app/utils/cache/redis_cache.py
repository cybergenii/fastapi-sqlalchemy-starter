...
class CacheManager:
    ...
    async def try_set_nx(self, key: str, expire: int = 86400) -> bool:
        """Set key only if absent. Returns True when this caller acquired the key."""
        if not self.enabled or not self.redis:
            return True
        ...
        try:
            ...
            result = await asyncio.wait_for(
                self.redis.set(key, b"1", nx=True, ex=expire),
                timeout=5,
            )
            return bool(result)
        except Exception as e:
            logs.warning(f"Redis SET NX failed for key {key}: {e}")
            return True
