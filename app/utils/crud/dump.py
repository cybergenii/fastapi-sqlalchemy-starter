...
ENABLE_PERF_LOGS = False  # Set to False in production
        ...
        async def wrapper(self, *args, **kwargs):
            ...
            try:
                ...
                if isinstance(result, dict) and "data" in result:
                    if hasattr(result["data"], "__iter__") and not isinstance(
                        result["data"], (str, bytes)
                    ):
                        # Convert list of SQLAlchemy objects to list of dicts
                        serializable_data = []
                        for item in result["data"]:
                            if hasattr(item, "to_dict") and not isinstance(
                                item, (dict, str, bytes, int, float, bool)
                            ):
                    ...
                    elif hasattr(result["data"], "to_dict") and not isinstance(
                        result["data"], (dict, str, bytes, int, float, bool)
                    ):
                ...
                elif hasattr(result, "to_dict") and not isinstance(
                    result, (dict, str, bytes, int, float, bool)
                ):
...
class OptimizedCrudService:
    ...
    def _build_populate_options(self, populate: List[PopulateField]) -> List[Any]:
        ...
        log.logs.info(
            f"📊 [PERF] Building populate options for {len(populate)} relationships"
        )
        ...
        for i, pop_config in enumerate(populate):
            ...
            if cache_key in self._relationship_cache:
                ...
                cache_misses += 1
                relationship_build_time = (
                    time.time() - relationship_build_start
                ) * 1000
                log.logs.debug(
                    f"🔧 [PERF] Built {load_type} for {path}: {relationship_build_time:.2f}ms"
                )
            ...
            if second_layer:
                ...
                log.logs.debug(
                    f"🔄 [PERF] Processing {len(second_layer)} nested relationships for {path}"
                )
                for nested_config in second_layer:
                    ...
                    if nested_path:
                        ...
                        log.logs.debug(
                            f"🔗 [PERF] Chained {nested_type} for {path}.{nested_path}"
                        )

                nested_time = (time.time() - nested_start) * 1000
                log.logs.debug(
                    f"⏱️ [PERF] Nested relationships for {path}: {nested_time:.2f}ms"
                )
    ...
    async def get_many(
        ...
        try:
            ...
            log.logs.info(
                f"⏱️ [PERF] Query model creation: {(time.time() - query_build_start) * 1000:.2f}ms"
            )
            ...
            if populate:
                ...
                log.logs.info(
                    f"⏱️ [PERF] Populate options built: {(time.time() - populate_start) * 1000:.2f}ms"
                )
            else:
                log.logs.info(
                    f"⏱️ [PERF] No populate options: {(time.time() - populate_start) * 1000:.2f}ms"
                )
            ...
            if filter:
                ...
                log.logs.info(
                    f"⏱️ [PERF] Additional filters applied: {(time.time() - filter_start) * 1000:.2f}ms"
                )
            else:
                log.logs.info(
                    f"⏱️ [PERF] No additional filters: {(time.time() - filter_start) * 1000:.2f}ms"
                )
            ...
            log.logs.info(
                f"⏱️ [PERF] Query processing (filter/paginate/sort): {(time.time() - query_processing_start) * 1000:.2f}ms"
            )
            ...
            try:
                compiled_query = str(
                    optimized_query.model.compile(
                        compile_kwargs={"literal_binds": True}
                    )
                )
                log.logs.info(
                    f"🔍 [PERF] SQL Query: {compiled_query[:500]}{'...' if len(compiled_query) > 500 else ''}"
                )
            except Exception as e:
            ...
            try:
                ...
                if hasattr(engine, "pool"):
                    ...
                    log.logs.info(
                        f"🔗 [PERF] DB Pool - Size: {pool_size}, CheckedIn: {checked_in}, CheckedOut: {checked_out}, Overflow: {overflow}"
                    )
                else:
            ...
            if not results:
                ...
                log.logs.info(
                    f"✅ [PERF] get_many completed (no data): {total_time:.2f}ms"
                )
            ...
            if populate:
                log.logs.info(
                    f"🔄 [PERF] Starting post-processing for {len(populate)} populate fields"
                )
                ...
                for i, result in enumerate(results):
                    ...
                    if i % 10 == 0:  # Log every 10th conversion
                        log.logs.debug(
                            f"⏱️ [PERF] Converted item {i}: {(time.time() - result_start) * 1000:.2f}ms"
                        )

                conversion_time = (time.time() - conversion_start) * 1000
                log.logs.info(
                    f"⏱️ [PERF] Dictionary conversion: {conversion_time:.2f}ms"
                )
                log.logs.info(
                    f"📊 [PERF] Converted {len(converted_results)} items to dictionaries"
                )
                results = converted_results
            else:
                log.logs.info(
                    f"⏱️ [PERF] No post-processing needed: {(time.time() - post_processing_start) * 1000:.2f}ms"
                )
            ...
            log.logs.info(
                f"   ⏱️ DB execution: {db_execution_time:.2f}ms ({db_execution_time / total_time * 100:.1f}%)"
            )
            log.logs.info(
                f"   ⏱️ Post-processing: {post_processing_time:.2f}ms ({post_processing_time / total_time * 100:.1f}%)"
            )
            if populate:
        ...
        try:
            ...
            if populate:
                ...
                if hasattr(db_item_selected, "model_dump"):
                    ...
                    db_item_selected = (
                        {"id": str(db_item_selected.id)}
                        if hasattr(db_item_selected, "id")
                        else {}
                    )
    ...
    def _filter_populated_fields(
        ...
        filter_start = time.time()
        log.logs.info(
            f"🔍 [PERF] Starting field filtering for {len(populate)} populate fields"
        )

        if not result or not populate:
            log.logs.info(
                f"⏱️ [PERF] No filtering needed: {(time.time() - filter_start) * 1000:.2f}ms"
            )
            return result
            ...
            try:
                ...
                if isinstance(single_obj, dict):
                    ...
                    try:
                        ...
                        all_columns = (
                            set(single_obj.__dict__.keys())
                            if hasattr(single_obj, "__dict__")
                            else set()
                        )
    ...
    def _convert_to_dict_with_relationships(
        ...
        conversion_start = time.time()
        log.logs.debug(
            f"🔄 [PERF] Converting object to dict with {len(populate)} populate fields"
        )
        ...
        if hasattr(obj, "to_dict"):
            ...
            log.logs.debug(
                f"⏱️ [PERF] to_dict() conversion: {dict_conversion_time:.2f}ms"
            )
            ...
            for pop_config in populate:
                ...
                if path and hasattr(obj, path):
                    ...
                    log.logs.debug(
                        f"🔄 [PERF] Related object for {path}: {type(related_obj)}"
                    )
                    ...
                    if related_obj is not None:
                        if isinstance(related_obj, list):
                            ...
                            log.logs.debug(
                                f"🔄 [PERF] Converted collection {path} with {len(related_obj)} items"
                            )
                        else:
                            ...
                            if hasattr(related_obj, "to_dict"):
                                ...
                                log.logs.debug(
                                    f"🔄 [PERF] Converted single relationship {path} to {relationship_name}"
                                )
                            else:
                                relationship_name = (
                                    path.split("__")[-1] if "__" in path else path
                                ...
                                log.logs.debug(
                                    f"🔄 [PERF] Used ID for {relationship_name}"
                                )

            relationship_processing_time = (
                time.time() - relationship_processing_start
            ) * 1000
            log.logs.debug(
                f"⏱️ [PERF] Relationship processing: {relationship_processing_time:.2f}ms"
            )
        else:
                ...
                for key, value in obj.__dict__.items():
                    if not key.startswith("_"):  # Skip private attributes
                        if hasattr(value, "isoformat"):  # Handle datetime objects
                            ...
                            data[key] = value
                log.logs.debug(
                    f"🔄 [PERF] Converted using __dict__ with {len(data)} fields"
                )
            else:
            ...
            dict_conversion_time = (time.time() - dict_conversion_start) * 1000
            log.logs.debug(
                f"⏱️ [PERF] Manual dict conversion: {dict_conversion_time:.2f}ms"
            )

        total_conversion_time = (time.time() - conversion_start) * 1000
        log.logs.debug(
            f"✅ [PERF] Object conversion completed: {total_conversion_time:.2f}ms ({len(data)} fields)"
        )
        return data
